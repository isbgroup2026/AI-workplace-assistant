...
export default function Dashboard({
  ...
  const taskEvents = tasks.map((t) => ({
    text:
      t.status === 'Done'
        ? `${t.assignee} completed "${t.title}"`
        : t.status === 'In Progress'
        ? `${t.assignee} is working on "${t.title}"`
        : `${t.assignee} has a pending task: "${t.title}"`,
  ...
  const meetingEvents = meetings
    ...
    .map((m) => ({
      text: `Meeting "${m.title}" was ${m.status.toLowerCase()}`,
      timestamp: m.updatedAt,
    }))

  const recentActivity = [...taskEvents, ...meetingEvents]
    .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
    .slice(0, 6)
    .map((e) => ({ text: e.text, time: timeAgo(e.timestamp) }))
    ...
    <div className="p-6 space-y-6 max-w-6xl">
      <div>
        ...
        <p className="text-sm text-inkmuted mt-1">
          Here's what's moving across {user.plant} today,{' '}
        ...
        <Card className="p-5 lg:col-span-2">
          ...
          <ul className="space-y-4">
            ...
            {recentActivity.length === 0 && (
              <li className="text-sm text-inkmuted py-2">No recent activity yet.</li>
            )}
          </ul>
        </Card>
