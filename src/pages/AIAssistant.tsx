...
import { suggestedPrompts, initialConversation, getMockAIResponse } from '../data/ai'
import { Avatar } from '../components/ui'
import { listAIMessages, saveAIMessage } from '../lib/api'
...
export default function AIAssistant({ user, myId }: { user: CurrentUser; myId: string }) {
  ...
  function sendMessage(text: string) {
    ...
    setConversations((prev) => ({ ...prev, [language]: [...prev[language], userMsg] }))
    saveAIMessage(myId, 'user', text)
    setInput('')
    setIsTyping(true)

    setTimeout(() => {
      const responseText = getMockAIResponse(text)
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] max-w-4xl mx-auto w-full">
      <div className="px-6 pt-5 pb-3 flex items-center justify-between border-b border-line">
        <div>
          ...
          <p className="text-xs text-inkmuted mt-0.5">Mock responses for demo purposes · no live AI connected</p>
        </div>
